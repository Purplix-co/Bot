...
def load_mods():
    ...
    for filename in sorted(os.listdir(MODS_DIR)):
        ...
        try:
            # Загружаем мод как отдельный модуль
            import importlib.util
            spec = importlib.util.spec_from_file_location(mod_name, mod_path)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            
            # Если у мода есть функция start() — запускаем её
            if hasattr(mod, "start"):
                mod.start()
                mods.append(mod_name)
            else:
                print(f"⚠️ Мод {mod_name} загружен, но нет функции start()")
        
        except Exception as e:
            print(f" Error {mod_name}: {e}")

# === ВЫПОЛНЕНИЕ КОМАНДЫ ИЗ МОДА ===
def run_mod_command(command, arg):
    """Ищет команду во всех модах и выполняет"""
    for mod_name, commands in loaded_mods.items():
        if command in commands:
            try:
                commands[command](arg)
                return True
            except Exception as e:
                print(f"❌ Ошибка в моде {mod_name}: {e}")
                return True
    return False

# === СПИСОК ВСЕХ КОМАНД МОДОВ ===
def get_mod_commands():
    """Возвращает список всех команд из модов"""
    all_commands = []
    for mod_name, commands in loaded_mods.items():
        for cmd in commands:
            all_commands.append(f"  {cmd} (mod: {mod_name})")
    return all_commands
